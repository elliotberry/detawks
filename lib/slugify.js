...
const stringModificationFunctions = {
    ...
    maxChars: (str, max = 100) => {
        return str.substring(0, max)
    },
    minChars: (str, min = 3) => {
        return str.substring(min)
    },
    removeConsecutiveDashes: (str) => {
        return str.replace(/-+/g, '-')
    },
    removeConsecutiveDashesAtStartEnd: (str) => {
        return str.replace(/^-+|-+$/g, '')
    },
    fallbackToRandom: (str) => {
        if (str === '') {
...
const init = () => {
    ...
    arrayOfFunctions = slugifyConfig.map((fn) => {
        if (stringModificationFunctions[fn] === undefined) {
            throw new Error(`Function ${fn} is not defined`)
        } else {
            return stringModificationFunctions[fn]
        }
    })
}

const slugify = (str) => {
    str = pipeFunctions(str, ...arrayOfFunctions)
    if (str === '') {
